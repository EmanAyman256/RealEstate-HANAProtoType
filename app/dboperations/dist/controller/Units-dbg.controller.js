...
onOpenPaymentSimulation: function (oEvent) {
    ...
    if (!this._oSimulationDialog) {
        ...
        var oVBox = new sap.m.VBox({
            items: [
                ...
                new sap.m.Input({  // Read-only, auto-filled from unit conditions
                    id: "pricePlanInputPPS",
                    value: "{local>/pricePlanYears}",  // Bind to calculated years
                    editable: false,  // Prevent manual input
                    placeholder: "Auto-calculated from unit conditions"
                }),
    ...
    if (this._oSimulationDialog) {
        ...
        if (unit) {
            ...
            // Calculate total years from unit's conditions
            const unitConditions = unit.conditions || [];
            const totalYears = unitConditions.reduce((sum, condition) => sum + (condition.numberOfYears || 0), 0);
            oLocal.setProperty("/pricePlanYears", totalYears);

            // Open the Price Plan Value Help dialog (filtered by project and years)
            this.onOpenPricePlanValueHelpPPS();
        }
...
// Updated: Value help for Price Plan (Years) - filtered by project AND calculated years from unit conditions
onOpenPricePlanValueHelpPPS: function () {
    ...
    const pricePlanYears = this._oSimulationDialog.getModel("local").getProperty("/pricePlanYears");  // Get calculated years from unit conditions
    console.log("Opening Price Plan Value Help for projectId:", projectId, "and years:", pricePlanYears);  // Debug log

    if (!this._oPricePlanValueHelpPPS) {
        this._oPricePlanValueHelpPPS = new SelectDialog({
            title: "Select Payment Plan (Filtered by Unit Conditions Years)",
            items: {
                path: "filteredPlans>/",
    ...
    // Filter items by current project AND exact match for calculated years from unit conditions
    ...
    const filteredPlans = aPlans.filter(p =>
        ...
        p.planYears === pricePlanYears  // NEW: Exact match for calculated years from unit conditions
    );
    console.log("Filtered plans for project and years:", filteredPlans);  // Debug log

    if (filteredPlans.length === 0) {
        sap.m.MessageBox.warning(`No payment plans available for the selected project with ${pricePlanYears} years from unit conditions.`);
