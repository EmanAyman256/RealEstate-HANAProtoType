...
onSavePlan: async function () {
    ...
    try {
        ...
        // Define planYears for validations
        const planYears = parseInt(oData.planYears) || 0;

        // Validation: Total percentage must be 100 (excluding maintenance)
        const totalPercentage = (oData.schedules || [])
            .filter(s => !(s.conditionType?.description?.toLowerCase().includes('maintenance')))
            .reduce((sum, s) => sum + (parseFloat(s.percentage) || 0), 0);
        if (totalPercentage !== 100) {
            MessageBox.error(`Total percentage for installments and downpayment must be 100. Current: ${totalPercentage}`);
            return;  // Prevent save
        }
